...
:- object('1.2.1',
    ...
    subcontent(
        [ p("An atom is either:")
        , ol(class('list-group'),
            [ li(class('list-group-item'), ["A string of characters made up of upper-case letters, lower-case letters, digits, and the underscore character, that begins with a lower-case letter. Here are some examples: ", \inline_code("butch"), ", ", \inline_code("big_kahuna_burger"), ", ", \inline_code("listens2Music"), " and ", \inline_code("playsAirGuitar"), "."])
            , li(class('list-group-item'), ["An arbitrary sequence of characters enclosed in single quotes. For example ", \inline_code("'Vincent'"), ", ", \inline_code("'The Gimp'"), ", ", \inline_code("'Five_Dollar_Shake'"), ",  ", \inline_code("'&^%&#@$ &*'"), ", and ", \inline_code("' '"), ". The sequence of characters between the single quotes is called the atom name. Note that we are allowed to use spaces in such atoms; in fact, a common reason for using single quotes is so we can do precisely that."])
            , li(class('list-group-item'), ["A string of special characters. Here are some examples: ", \inline_code("@="), " and ", \inline_code("====>"), " and ", \inline_code(";"), " and ", \inline_code(":-"), " are all atoms. As we have seen, some of these atoms, such as ", \inline_code(";"), " and ", \inline_code(":-"), " have a pre-defined meaning."])
            ])
        ]
    ).
:- end_object.
